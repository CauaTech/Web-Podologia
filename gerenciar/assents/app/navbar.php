...
$sql_notificy = "SELECT * FROM `vortex-notificacao` ORDER BY `id` DESC";
$f = $connect->query($sql_notificy);
	    	...
	    	<ul class="navbar-nav">
		    	<div class="dropdown dropstart">
				  ...
				  <ul class="dropdown-menu">
				  	<?php 
				  	$pao = 0;
				  	while ($consult = $f->fetch_array()) {?>
				  		<?php 	
				  		$pao = $pao + 1;
				  		$token = $consult['token-paciente'];

				  		$sql_pa = "SELECT * FROM `vortex-pacientes` WHERE `paciente_token`='$token'";
				  		$kd = $connect->query($sql_pa);
				  		$cm = $kd->fetch_array();
				  		if ($pao == 7) {
				  			break; 
				  		}
				  		...
				  		?>
				    	<li><a class="dropdown-item" href="#"><?php echo $cm['Nome']; ?> Agendou <?php echo $consult['agenda-data'] ?> as <?php echo $consult['agenda-horario'] ?> - <span class="badge bg-success">Agendou</span></a></li>
				    <?php } ?>
				  </ul>
				</div>
